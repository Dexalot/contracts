...
/**
 * @title Mock contract to test multi endpoint connection.
 * @dev send() will short circuit to lzReceive() directly.
 * There is no reentrancy guard. The real LayerZero endpoint on mainnet has a send and receive guard.
 * if we run a ping-pong-like application, the recursive call might use all gas limit in the block.
 * This mock contract is not using any messaging library, hence all messaging library func, e.g. estimateFees,
...
contract LZEndpointMock is ILayerZeroEndpoint {
    ...
    bool nextMsgBLocked;
    // version added by Cengiz for compatibility
    bytes32 public constant VERSION = bytes32("1.9.0");
    ...
    mapping(uint16 => mapping(bytes => StoredPayload)) public override storedPayload;
    // msgToDeliver = [srcChainId][srcAddress]
    mapping(uint16 => mapping(bytes => QueuedPayload[])) public msgsToDeliver;
    // configs = [chainId][version][configType]
    mapping(uint16 => mapping(uint16 => mapping(uint256 => bytes))) private configs;
    // receiveVersion = [userApplication]
    mapping(address => uint16) private receiveVersion;
    // sendVersion = [userApplication]
    mapping(address => uint16) private sendVersion;
    ...
    function send(
        ...
        // Mock the relayer paying the dstNativeAddr the amount of extra native token
        {
            ...
            // to simulate actually sending the ether, add a transfer call and
            // ensure the LZEndpointMock contract has an ether balance
        }

        bytes memory bytesSourceUserApplicationAddr = addrToPackedBytes(address(msg.sender)); // cast address to bytes

        // not using the extra gas parameter because this is a single tx call, not split between different chains
        // LZEndpointMock(lzEndpoint).receivePayload(mockChainId, bytesSourceUserApplicationAddr, destAddr,
        //           nonce, extraGas, _payload);
    ...
    function receivePayload(
        ...
        if (sp.payloadHash != bytes32(0)) {
            ...
            // ILayerZeroReceiver(_dstAddress).lzReceive{gas: _gasLimit}(_srcChainId, _srcAddress, _nonce, _payload);
            // invoke lzReceive
    ...
    function setConfig(
        ...
        configs[_chainId][_version][_configType] = _config;
    }
    ...
    function getConfig(
        ...
        return configs[_chainId][_version][_configType];
    }

    function setSendVersion(uint16 _version) public override {
        sendVersion[address(this)] = _version;
    }

    function setReceiveVersion(uint16 _version) public override {
        receiveVersion[address(this)] = _version;
    }

    function getSendVersion(address _ua) external view override returns (uint16) {
        return sendVersion[_ua];
    }

    function getReceiveVersion(address _ua) external view override returns (uint16) {
        return receiveVersion[_ua];
    }
