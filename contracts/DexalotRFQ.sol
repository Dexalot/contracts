...
import "@openzeppelin-v5/utils/cryptography/EIP712.sol";
...
contract DexalotRFQ is IMainnetRFQ, AccessControlEnumerable, EIP712, IERC1271, ReentrancyGuard {
    ...
    address public immutable swapSigner;
    ...
    ) AccessControlEnumerable() EIP712("Dexalot", "2") {
    ...
    receive() external payable override {
        require(hasRole(REBALANCER_ADMIN_ROLE, msg.sender) || msg.sender == wrappedNative, "RF-RAOW-01");
    }
    ...
    /**
     * @notice  Allows rebalancer to send an asset to smart contract
     ...
     */
    function claimBalance(address _asset, uint256 _amount) external onlyRole(REBALANCER_ADMIN_ROLE) {
        if (_asset == address(0)) {
    ...
    function isValidSignature(bytes32 _hash, bytes calldata _signature) public view override returns (bytes4) {
        (address signer, , ) = ECDSA.tryRecoverCalldata(_hash, _signature);
    ...
    function _takeFunds(SwapData memory _swapData) private {
        if (_swapData.srcAsset == address(0)) {
            ...
            if (keepWrapped) {
                IWrappedToken(wrappedNative).deposit{value: _swapData.srcAmount}();
            }
            return;
        }

        if (_swapData.isDirect) {
            IERC20(_swapData.srcAsset).safeTransferFrom(_swapData.msgSender, address(this), _swapData.srcAmount);
        }
        if (!keepWrapped && _swapData.srcAsset == wrappedNative) {
            IWrappedToken(wrappedNative).withdraw(_swapData.srcAmount);
        }
    }
    ...
    function _releaseFunds(SwapData memory _swapData) private {
        ...
        if (destAsset == address(0)) {
            if (keepWrapped) {
                IWrappedToken(wrappedNative).withdraw(_swapData.destAmount);
            }
            (bool success, ) = payable(destTrader).call{value: _swapData.destAmount}("");
            require(success, "RF-TF-01");
        } else {
            if (!keepWrapped && destAsset == wrappedNative) {
                IWrappedToken(wrappedNative).deposit{value: _swapData.destAmount}();
            }
            IERC20(destAsset).safeTransfer(destTrader, _swapData.destAmount);
        }
    }
    ...
    function _processXFerPayloadInternal(
        ...
        if (_symbol == IPortfolio(portfolioMain).getNative()) {
            ...
            address token = address(IPortfolioMain(portfolioMain).getToken(_symbol));
            require(token != address(0), "RF-DTNF-01");

            (bool erc20Success, bytes memory data) = token.call(
                abi.encodeWithSelector(IERC20(token).transfer.selector, _trader, _quantity)
            );
