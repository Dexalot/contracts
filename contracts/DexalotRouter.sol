...
contract DexalotRouter is AccessControlEnumerable {
    ...
    fallback() external payable {
        ...
        if (msg.value == 0) {
            IERC20 takerAsset;
            uint256 amount;
            uint256 amountOffset = selector == PARTIAL_SWAP_SELECTOR
                ? TAKER_PARTIAL_AMOUNT_OFFSET
                : TAKER_AMOUNT_OFFSET;
            assembly {
            ...
            // Transfer source asset from msg.sender to target mainnetRFQ contract
            takerAsset.safeTransferFrom(msg.sender, targetImplementation, amount);
        }
