...
describe("Staking", function () {
	...
	let duration = 5184000;  // rewardsDuration = 60 days = 60 * 60 * 24 * 60 = 5184000

	async function deployStaking() {
		Staking = await ethers.getContractFactory("Staking");
		staking = await upgrades.deployProxy(Staking, [stakingToken.address, stakingToken.address, rewardRate, duration])
	...
	describe('Pausable', async () => {
		...
		it('should revert calling restake() when staking paused', async () => {
			await staking.unpause();
			await staking.pauseStaking();
	...
	describe('setRewardRate()', () => {
		...
		it('sets reward rate correctly', async () => {
