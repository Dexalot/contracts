...
describe("Portfolio Shared", () => {
    ...
    it("Should add and remove admin correctly", async function () {
        ...
        await expect(portfolio.revokeRole(role, ethers.constants.AddressZero)).to.revertedWith("P-OACC-02");
        // succeed for admin
        ...
        await expect(portfolio.revokeRole(await portfolio.PORTFOLIO_BRIDGE_ROLE(), owner.address)).to.be.revertedWith("P-ALOA-02");
    ...
    it("Should have gas Swap Amount 1 and bridgeFee 0 for ALOT both in PMain and PSub", async () => {
        ...
        //  ALOT bridgeFee can be changed, but Can't change ALOT gasSwapRatio(Silent fail)
    ...
    it("Should have gas Swap Amount 0.01 and bridgeFee 0 for AVAX both in PMain and PSub", async () => {
        ...
        expect(minAmounts[0]).includes(ALOT);
        expect(minAmounts[0]).includes(AVAX);
        expect(minAmounts[0]).not.includes(Utils.fromUtf8("USDT"));
    ...
    it("Should be able to set gasSwap Ratio to 0 in PortfolioSub for an auctionToken but not In PortfolioMain", async () => {
        ...
        await expect (f.addToken(portfolio, portfolioSub, usdt, 0, 0)).to.revertedWith("P-GSRO-01");
        expect ( (await portfolio.getTokenDetails(USDT)).auctionMode).to.be.equal(0);

        //  add erc20 to portfolioMain & Sub with 0.1 gasSwapRatio with an auctionmode 2
        await f.addToken(portfolio, portfolioSub, usdt, 0.1, 2);
    ...
    it("Should add, check and remove an PORTFOLIO_BRIDGE_ROLE correctly", async function () {
        ...
        const role = await portfolio.PORTFOLIO_BRIDGE_ROLE();
        // ADD
        // fail for non-admin
