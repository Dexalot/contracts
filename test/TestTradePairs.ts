...
async function  addOrderAndVerify(trader: SignerWithAddress, order: any, expectedStatus: number
    ...
    , expectedTotalAmount = BigNumber.from(0), expectedQuantityFilled = BigNumber.from(0), expectedTotalFee = BigNumber.from(0), m_debug=false
    ): Promise<string | any>{
    ...
    for (const e of res.events) {
        ...
        if (e.event === "OrderStatusChanged") {
            ...
            if (e.args.traderaddress === trader.address && e.args.order.clientOrderId == order.clientOrderId) {
                ...
                const decimals = e.args.order.side == 0 ? baseDecimals : quoteDecimals;
                if (m_debug) {
                    console.log(e.args.traderaddress.slice(-4), "Taker Order, Side", e.args.order.side, " qty filled", Utils.formatUnits(e.args.order.quantityFilled, baseDecimals), "feePaid", Utils.formatUnits(e.args.order.totalFee, decimals));
                }
                ordid = e.args.order.id;
            } else {
                if (m_debug) {
                    const decimals = e.args.order.side == 0 ? baseDecimals : quoteDecimals;
                    console.log(e.args.traderaddress.slice(-4), "Maker Order, Side", e.args.order.side, " qty filled", Utils.formatUnits(e.args.order.quantityFilled, baseDecimals), "feePaid", Utils.formatUnits(e.args.order.totalFee, decimals));
                }
            }
        } else if (e.event === "Executed") {
            //console.log(e.args)
            if (m_debug) {
                const decimals = e.args.takerSide == 0 ? baseDecimals : quoteDecimals;
                console.log(e.args.addressTaker.slice(-4), "Execution, Side", e.args.takerSide, " qty filled", Utils.formatUnits(e.args.quantity, baseDecimals), "feePaid", Utils.formatUnits(e.args.feeTaker, decimals));
                const decimalsMaker = e.args.takerSide == 1 ? baseDecimals : quoteDecimals;
                console.log(e.args.addressMaker.slice(-4), "Execution, Side", e.args.takerSide == 0 ? 1 : 0, " qty filled", Utils.formatUnits(e.args.quantity, baseDecimals), "feePaid", Utils.formatUnits(e.args.feeMaker, decimalsMaker));
            }
...
beforeEach(async function () {
    ...
    it("Should set min trade amount from the owner account", async function () {
        ...
        // minPostAmount initializes as teh same as MinTradeAmount =10
        // Can't set it more than 10
        await expect(tradePairs.connect(owner).setMinTradeAmount(tradePairId, minTradeAmount1)).to.be.revertedWith("T-MTMP-01");
        // succeed from owner accounts
        minTradeAmount1 = Utils.parseUnits('5', quoteDecimals);
    ...
    it("Should set min post amount from the owner account", async function () {
        ...
        let minPostAmount = Utils.parseUnits('5', quoteDecimals);
        // fail from non owner accounts
        await expect(tradePairs.connect(trader1).setMinPostAmount(tradePairId, minPostAmount)).to.be.revertedWith("AccessControl:");
        // minPostAmount initializes as teh same as MinTradeAmount =10
        // Can't set it less than 10
        await expect(tradePairs.connect(owner).setMinPostAmount(tradePairId, minPostAmount)).to.be.revertedWith("T-MTMP-01");
        // succeed from owner accounts
        minPostAmount = Utils.parseUnits('50', quoteDecimals);
        // succeed from owner accounts
    ...
    it("Should be able to add a new buy order from the trader accounts", async function () {
        ...
        await addOrderAndVerify(trader1, buyOrder, 1, Utils.fromUtf8("T-AUCT-02"));  // status is REJECTED = 1
    ...
    it("Should be able to handle dust fees & cancel dust maker orders from very small taker orders", async function () {

        const minTradeAmount = 0.001;
        const maxTradeAmount = 2000;
        const mode = 0;  // auction off
        const defaultTokenDeposit = '3000'
        const defaultNativeDeposit = '100';
        const pairSettings = {
            minTradeAmount: minTradeAmount,
            maxTradeAmount: maxTradeAmount,
            mode: mode,
        }
        // keeping very low rates to test out the edge cases
        const mRate = 2 //bps
        const tRate = 1
        // mint some tokens for trader1
        await quoteToken.mint(trader1.address, Utils.parseUnits('10000', quoteDecimals));
        ...
        const addresses: string[] = [trader1.address, trader2.address, feeSafe.address];

        const quoteBalances: any[3] = [];
        const baseBalances: any[3] = [];
        for (let i = 0; i < addresses.length; i++) {
            const addr = addresses[i];
            const quoteBal = await portfolio.getBalance(addr, quoteSymbol);
            const baseBal = await portfolio.getBalance(addr, baseSymbol);
            quoteBalances[i]= { total: BN(Utils.formatUnits(quoteBal.total, quoteDecimals)), available: BN(Utils.formatUnits(quoteBal.available, quoteDecimals)) };
            baseBalances[i]={ total: BN(Utils.formatUnits(baseBal.total, baseDecimals)), available: BN(Utils.formatUnits(baseBal.available, baseDecimals)) };

            if (i == 0) { // Maker order total should always be equal to available at the begining
                expect(quoteBal.total).to.be.eq(quoteBal.available);
                expect(baseBal.total).to.be.eq(baseBal.available);
            }

            if (i == 1) { // Taker order total should always be equal to available
                expect(quoteBal.total).to.be.eq(quoteBal.available);
                expect(baseBal.total).to.be.eq(baseBal.available);
            }

            if (i == 2) { // FeeSafe starts with 0 balances
                expect(quoteBal.total).to.be.eq(0);
                expect(baseBal.total).to.be.eq(0);
            }

            if (debug) {
                console.log("T", i, "Quote Total", Utils.formatUnits(quoteBal.total, quoteDecimals), "available", Utils.formatUnits(quoteBal.available, quoteDecimals));
                console.log("T", i, "Base Total", Utils.formatUnits(baseBal.total, baseDecimals), "available", Utils.formatUnits(baseBal.available, baseDecimals));
            }
            i++;
        }

        if (debug) {
            console.log("Quote Bals",quoteBalances.map((obj: { toString: () => any; }) => JSON.stringify(obj)));
            console.log("Base Bals",baseBalances.map((obj: { toString: () => any; }) => JSON.stringify(obj)));
        }
        ...
        buyOrder.price = Utils.parseUnits(buyPx.toString(), quoteDecimals)
        buyOrder.quantity= Utils.parseUnits(buyQty.toString(), baseDecimals)
        //buyOrder.price(0.1) * quantity(6) 0.6 > 0.01 (0.001*10 minPost) > 0.001(minTrade)
        await addOrderAndVerify(trader1, buyOrder, 0 );  // status is NEW = 0
        // {GTC, FOK, IOC, PO}

        let trader1QuoteBal = await portfolio.getBalance(trader1.address, quoteSymbol);

        if (debug) {
            console.log("T1 Quote Total", Utils.formatUnits(trader1QuoteBal.total, quoteDecimals), "available", Utils.formatUnits(trader1QuoteBal.available, quoteDecimals));
        }


        // Try to post (no fills) an order smaller than minPostAmount, gets a cancel
        let sellQty = 0.001;
        let sellPx = 1.1;
        sellOrder.quantity = Utils.parseUnits(sellQty.toString(), baseDecimals);
        sellOrder.price = Utils.parseUnits(sellPx.toString(), quoteDecimals)
        ...
        // fill a very small limit order with the outstanding maker
        // minTradeAmount = 0.001;
        sellQty = 0.011;
        sellPx = 0.1

        let totalAmount = BN(sellPx).times(sellQty).dp(quoteDecimals);
        let sellFeeCalculated = totalAmount.times( tRate).div(10000).dp(quoteDecimals,BN.ROUND_UP); // returns 0 here,  but if rounded up it matches minum 1 unit fee in solidity
        let feeCalculatedParsed = Utils.parseUnits(sellFeeCalculated.toString(), quoteDecimals) //100 * 0.06 * 0.003
        let totalAmountParsed = Utils.parseUnits(totalAmount.toString(), quoteDecimals) //100 * 0.06 * 0.003



        let makerQuantityFilled = BN(sellQty);
        let makerTotalAmount = totalAmount;
        let makerFeeCalculated = BN(sellQty).times(mRate).div(10000).dp(baseDecimals);

        if (debug) {
            console.log("1st taker fill", sellQty, "fee", sellFeeCalculated.toString(), "feeParsed", feeCalculatedParsed.toString(), "total", totalAmount.toString(), "totalParsed", totalAmountParsed.toString());
            console.log("1st maker fill. QtyFilled So far", makerQuantityFilled.toString(), "fee", makerFeeCalculated.toString(),  "totalAmount", makerTotalAmount.toString());
        }
        sellOrder.price = Utils.parseUnits(sellPx.toString(), quoteDecimals);
        sellOrder.quantity = Utils.parseUnits(sellQty.toString(), baseDecimals);
        // await addOrderAndVerify(trader2, sellOrder, 3, ethers.constants.HashZero, totalAmountParsed
        //     , sellOrder.quantity ,feeCalculatedParsed);  //  status is FILLED = 3 ,feeCalculatedParsed


        let tx = await tradePairs.connect(trader2).addNewOrder(sellOrder);

        let res: any = await tx.wait();

        for (const e of res.events) {
            if (e.event === "OrderStatusChanged" ){
                if (e.args.traderaddress === trader1.address) {  // This is the resting order(maker)
                    ...
                    expect(e.args.order.price).to.be.equal(buyOrder.price);
                    expect(e.args.order.totalAmount).to.be.equal(Utils.parseUnits(makerTotalAmount.toString(), quoteDecimals));
                    expect(e.args.order.quantity).to.be.equal(buyOrder.quantity);
                    expect(e.args.order.side).to.be.equal(buyOrder.side);              // side is SELL=1
                    expect(e.args.order.type1).to.be.equal(buyOrder.type1);             // type1 is LIMIT=1
                    expect(e.args.order.type2).to.be.equal(buyOrder.type2);            // type2 is GTC=0 ( FOK-is ignored!!!)
                    expect(e.args.order.status).to.be.equal(2);            // status is PARTIAL = 2
                    expect(e.args.order.quantityFilled).to.be.equal(Utils.parseUnits(makerQuantityFilled.toString(), baseDecimals));
                    expect(e.args.order.totalFee).to.be.equal(Utils.parseUnits(makerFeeCalculated.toString(), baseDecimals));
                    if (debug) {
                        const decimals = e.args.order.side == 0 ? baseDecimals : quoteDecimals;
                        console.log(e.args.traderaddress.slice(-4), "Maker Order qty filled", Utils.formatUnits(e.args.order.quantityFilled, baseDecimals), "feePaid", Utils.formatUnits(e.args.order.totalFee, decimals));
                    }
                    ...
                    expect(e.args.order.totalAmount).to.be.equal(totalAmountParsed);  // totalamount is 1000 QT
                    expect(e.args.order.quantity).to.be.equal(sellOrder.quantity);
                    expect(e.args.order.side).to.be.equal(sellOrder.side);              // side is BUY=0
                    ...
                    expect(e.args.order.quantityFilled).to.be.equal(sellOrder.quantity);   // quantityfilled is 10 AVAX
                    expect(e.args.order.totalFee).to.be.equal(feeCalculatedParsed);
                    if (debug) {
                        const decimals = e.args.order.side == 0 ? baseDecimals : quoteDecimals;
                        console.log(e.args.traderaddress.slice(-4), "Taker Order, Side", e.args.order.side, " qty filled", Utils.formatUnits(e.args.order.quantityFilled, baseDecimals), "feePaid", Utils.formatUnits(e.args.order.totalFee, decimals));
                    }
        ...
        //CHeck balances
        const quoteBalancesNext: any[3] = [];
        const baseBalancesNext: any[3] = [];
        for (let i = 0; i < addresses.length; i++) {
            const addr = addresses[i];
            const quoteBal = await portfolio.getBalance(addr, quoteSymbol);
            const baseBal = await portfolio.getBalance(addr, baseSymbol);

            quoteBalancesNext[i]= { total: BN(Utils.formatUnits(quoteBal.total, quoteDecimals)), available: BN(Utils.formatUnits(quoteBal.available, quoteDecimals)) };
            baseBalancesNext[i]={ total: BN(Utils.formatUnits(baseBal.total, baseDecimals)), available: BN(Utils.formatUnits(baseBal.available, baseDecimals)) };
            if (debug) {
                console.log("New Quote Bal", JSON.stringify(quoteBalancesNext[i]));
                console.log("New Base Bal", JSON.stringify(baseBalancesNext[i]));
            }

            if (i == 0) {
                // Maker order buying , quote goes down base goes up , fee paid in base
                //maker number are already running sum, no need to update the quoteBalances & baseBalances
                expect(quoteBalances[i].available.minus(BN(buyQty).times(buyPx)).toString()).to.be.eq(quoteBalancesNext[i].available.toString());
                expect(quoteBalances[i].total.minus(makerTotalAmount).toString()).to.be.eq(quoteBalancesNext[i].total.toString());
                expect(baseBalances[i].total.plus(makerQuantityFilled).minus(makerFeeCalculated).toString()).to.be.eq(baseBalancesNext[i].total.toString());
                expect(baseBal.total).to.be.eq(baseBal.available);
            }

            if (i == 1) { // Taker order total should always be equal to available
                // Taker order selling , base goes down quote goes up
                expect(quoteBal.total).to.be.eq(quoteBal.available);
                expect(baseBal.total).to.be.eq(baseBal.available);
                expect(quoteBalances[i].total.plus(totalAmount).minus(sellFeeCalculated).toString()).to.be.eq(quoteBalancesNext[i].total.toString());
                expect(baseBalances[i].total.minus(sellQty).toString()).to.be.eq(baseBalancesNext[i].total.toString());

                // Store the new values
                quoteBalances[i] = quoteBalancesNext[i];
                baseBalances[i] = baseBalancesNext[i];

            }

            if (i == 2) {
                // Taker order selling, paying fee out in terms of quote
                expect(quoteBalancesNext[i].total.toString()).to.be.eq(quoteBalances[i].total.plus(sellFeeCalculated).toString());
                //resting order buyin and paying out in terms of base
                expect(baseBalancesNext[i].total.toString()).to.be.eq(makerFeeCalculated.toString());
                // expect(baseBal.total).to.be.eq(baseBalances[i].total.plus(Utils.parseUnits(makerFeeCalculated.toString(), baseDecimals)));
                // Store the quoteBalances values ONLY, makerFeeCalculated is already running sum
                quoteBalances[i] =quoteBalancesNext[i];
            }
        ...
        sellQty = 4.989;  // leave 1 qty in the resting order
        sellOrder.quantity = Utils.parseUnits(sellQty.toString(), baseDecimals);
        sellOrder.clientOrderId = await Utils.getClientOrderId(ethers.provider, trader2.address, 2);

        totalAmount =  BN(sellPx).times(sellQty);
        sellFeeCalculated = totalAmount.times(tRate).div(10000).dp(quoteDecimals,BN.ROUND_DOWN); // truncate after 6th digit default solidity behaviour

        feeCalculatedParsed = Utils.parseUnits(sellFeeCalculated.toString(), quoteDecimals)

        totalAmountParsed = Utils.parseUnits(totalAmount.toString(), quoteDecimals);

        makerQuantityFilled = makerQuantityFilled.plus(sellQty);
        makerTotalAmount = makerTotalAmount.plus(totalAmount);
        makerFeeCalculated = BN(sellQty).times(mRate).div(10000).dp(baseDecimals).plus(makerFeeCalculated);


        if (debug) {
            console.log("2nd Taker fill", sellQty, "fee", sellFeeCalculated.toString(), "feeParsed", feeCalculatedParsed.toString(), "total", totalAmount.toString(), "totalParsed", totalAmountParsed.toString());
            console.log("2nd maker fill. QtyFilled So far", makerQuantityFilled.toString(), "fee", makerFeeCalculated.toString(),  "totalAmount", makerTotalAmount.toString());
        }

        await addOrderAndVerify(trader2, sellOrder, 3, ethers.constants.HashZero, totalAmountParsed
            , sellOrder.quantity, feeCalculatedParsed, debug);  //  status is FILLED = 3
        ...
        // Very small maker left in the orderbook. Outstanding order Buy 1 @ 0.1 = 0.1
        // MinPost amount is 0.01
        // a resting order will be canceled if it is < minTradeAmount /10  which is 0.0001

        //First set the minTradeAmount to something very close to minPost to make the unsolicited cancel easier to achieve
        await tradePairs.connect(owner).setMinTradeAmount(tradePairId, Utils.parseUnits('0.009', quoteDecimals));
        expect((await tradePairs.getTradePair(tradePairId)).minTradeAmount).to.be.equal(Utils.parseUnits('0.009', quoteDecimals));
        ...
        if (debug) {
            console.log("3rd Taker fill", sellQty, "fee", sellFeeCalculated.toString(), "feeParsed", feeCalculatedParsed.toString(), "total", totalAmount.toString(), "totalParsed", totalAmountParsed.toString());
            console.log("3rd maker fill. QtyFilled So far", makerQuantityFilled.toString(), "fee", makerFeeCalculated.toString(),  "totalAmount", makerTotalAmount.toString());
        }

        tx = await tradePairs.connect(trader2).addNewOrder(sellOrder);
        ...
        for (const e of res.events) {
            if (e.event === "OrderStatusChanged" ){
                if (e.args.traderaddress === trader1.address) {  // This is the resting order(maker)
                    ...
                    expect(e.args.order.quantityFilled).to.be.equal(Utils.parseUnits(makerQuantityFilled.toString(), baseDecimals));
                    expect(e.args.order.totalFee).to.be.equal(Utils.parseUnits(makerFeeCalculated.toString(), baseDecimals));
                    expect(e.args.code).to.be.equal(Utils.fromUtf8("T-LTPA-02"));         // error code
        ...
        for (let i = 0; i < addresses.length; i++) {
            ...
            if (i == 0) {
                ...
                // THE REMAINING IS CANCELED so total bal is simply original - makerTotalAmount
                expect(quoteBalances[i].total.minus(makerTotalAmount).toString()).to.be.eq(quoteBalancesNext[i].total.toString());
                expect(baseBalances[i].total.plus(makerQuantityFilled).minus(makerFeeCalculated).toString()).to.be.eq(baseBalancesNext[i].total.toString());
                expect(baseBal.total).to.be.eq(baseBal.available);
            }
    ...
    it("Should be able to send addOrderList with autoGas", async function () {
        // mint some tokens for trader1
        await quoteToken.mint(trader1.address, Utils.parseUnits('10000', quoteDecimals));
        ...
        // autoGas kicked in, wallet balance less than what we started with after deposited gas
    ...
    it("Should autofill kick-in when adding & canceling an order", async function () {
        ...
        const gasDeposited = await gasStation.gasAmount();
        const qtSwappedAmnt = (await portfolio.bridgeParams(QT)).gasSwapRatio.mul(gasDeposited).div(BigNumber.from(10).pow(18))
        // Test out addOrder autoGas using QT
        const WalBaltoReset = gasDeposited.div(2);
        await f.setHardhatBalance(trader1, WalBaltoReset);
    ...
    it("Should autofill kick-in using ALOT when adding/canceling an order if ALOT is available", async function () {
        ...
        const WalBaltoReset =gasDeposited.div(2);
        await f.setHardhatBalance(trader1, WalBaltoReset);
        // Test out addOrder autoGas using a sell order AVAX/QT
    ...
    it("Should be able to add multiple orders and cancel them from the same trader account", async function () {
        // mint some tokens for trader1
        await quoteToken.mint(trader1.address, Utils.parseUnits('10000', quoteDecimals));
    ...
    it("Should use setAuctionVaultAdress correctly", async function () {
        // mint some tokens for trader1
        await quoteToken.mint(trader1.address, Utils.parseUnits('10000', quoteDecimals));
        // deposit some native to portfolio for trader1
        await f.depositNative(portfolioMain, trader1, defaultNativeDeposit)
        ...
        await expect(tradePairs.connect(trader1).setAuctionVaultAdress(tradePairId, owner.address))
                        .to.be.revertedWith("AccessControl:");
        // succeed for owner
        await tradePairs.connect(owner).setAuctionVaultAdress(tradePairId, trader1.address)
        expect((await tradePairs.getAuctionVaultAdress(tradePairId))).to.equal(trader1.address)
    });
    ...
    it("Should be able to use cancelOrder(), cancelOrderList() and cancelReplaceOrder() correctly", async function () {
        ...
        // mint some tokens for trader1
        await quoteToken.mint(auctionAdmin.address, Utils.parseUnits('20000', quoteDecimals));

        // deposit some native to portfolio for auctionAdmin
        await f.depositNative(portfolioMain, auctionAdmin, defaultNativeDeposit)
        expect((await portfolio.getBalance(auctionAdmin.address, baseSymbol))[0]).to.equal(Utils.parseUnits('3000', baseDecimals));
        expect((await portfolio.getBalance(auctionAdmin.address, baseSymbol))[1]).to.equal(Utils.parseUnits('3000', baseDecimals));

        // deposit some tokens to portfolio for auctionAdmin
        await f.depositToken(portfolioMain, auctionAdmin, quoteToken, quoteDecimals, quoteSymbol, defaultTokenDeposit)
        expect((await portfolio.getBalance(auctionAdmin.address, quoteSymbol))[0]).to.equal(Utils.parseUnits('2000', quoteDecimals));
        expect((await portfolio.getBalance(auctionAdmin.address, quoteSymbol))[1]).to.equal(Utils.parseUnits('2000', quoteDecimals));

        await f.addTradePair(exchange, pair, defaultPairSettings)

        let clientOrderid = await Utils.getClientOrderId(ethers.provider, auctionAdmin.address);
        // add a buy order
        buyOrder.price = Utils.parseUnits('1', quoteDecimals);
        buyOrder.quantity = Utils.parseUnits('100', baseDecimals);
        buyOrder.traderaddress = auctionAdmin.address;
        buyOrder.clientOrderId = clientOrderid;
        const tx1 = await tradePairs.connect(auctionAdmin).addNewOrder(buyOrder);
        ...
        await tradePairs.connect(owner).pause()
        await expect(tradePairs.connect(auctionAdmin)
        ...
        // mint some tokens for auctionAdmin
        ...
        tx2 = await tradePairs.connect(auctionAdmin).cancelOrder(id1)
        res2 = await tx2.wait();
        for (const e of res2.events) {
            if (e.event === "OrderStatusChanged" && e.args.traderaddress === auctionAdmin.address) {
        ...
        tx2 = await tradePairs.connect(auctionAdmin).cancelOrderList([id1])
        res2 = await tx2.wait();
        for (const e of res2.events) {
            if (e.event === "OrderStatusChanged" && e.args.traderaddress === auctionAdmin.address) {
                    expect(e.args.order.id).to.be.equal(id1)
        ...
        await tradePairs.connect(owner).setAuctionMode(tradePairId, 2);  // auction is OPEN
        clientOrderid = await Utils.getClientOrderId(ethers.provider, auctionAdmin.address);
        await expect(tradePairs.connect(auctionAdmin)
