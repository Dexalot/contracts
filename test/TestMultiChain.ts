...
describe("MultiChain Deployments & Interactions", () => {
    ...
    it("Should deposit ERC20 portfolioMain & withdraw from Subnet back to original host chain", async () => {
        ...
        const cChainfee = await portfolioSub.getBridgeFee(0, cChain.chainListOrgId, symbol, Utils.parseUnits(withdraw_amount, token_decimals), trader1.address, Utils.emptyOptions());
        ...
        const arbFee = await portfolioSub.getBridgeFee(0, arbitrumChain.chainListOrgId, symbol, Utils.parseUnits(withdraw_amount, token_decimals), trader1.address, Utils.emptyOptions());
