...
before(async () => {
  ...
  for (let j=0; j<tokenList.length; j++) {
      ...
      await portfolio.addToken(Utils.fromUtf8(await _token.symbol()), _token.address, srcChainId, await _token.decimals(), _startAuctionMode); //Auction mode
      await portfolioMain.addToken(Utils.fromUtf8(await _token.symbol()), _token.address, srcChainId, await _token.decimals(), _startAuctionMode); //Auction mode
      for (let i=0; i<numberOfAccounts; i++) {
...
it("Should give correct Portfolio state after ALL OUTSTADNING ORDERS CANCELED! All Portfolio Totals Should be equal to available", async () => {
