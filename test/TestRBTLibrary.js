...
describe("TestRBTLibrary", function () {
	let TestRBTLibrary
	let tree
	let keys

	before(async function () {
		TestRBTLibrary = await ethers.getContractFactory("TestRBTLibrary");
	});

	beforeEach(async function () {
		tree = await TestRBTLibrary.deploy();
	});

	describe('TestRBTLibrary', () => {

		it('should use isEmpty() correctly', async () => {
			expect(await tree.isEmpty(10)).to.be.equal(false);
			expect(await tree.isEmpty(0)).to.be.equal(true);
		});
		...
		it('should use exists() correctly', async () => {
			...
			expect(await tree.exists('1')).to.be.equal(false);
			// insert one element: true
			await tree.insert(1, 1);
			expect(await tree.exists(1)).to.be.equal(true);
		});
		...
		it('should use insert() correctly', async () => {
			...
			for (let i=1; i<21; i++) {
				expect(await tree.exists(i)).to.be.equal(true);
			}
			// empty key
		...
		it('should use remove() correctly', async () => {
			...
			// remove existing key
			expect(await tree.exists(15)).to.be.equal(true);
			await tree.remove(15)
			expect(await tree.exists(15)).to.be.equal(false);
			// reinsert 15
		...
		it('should use next() correctly', async () => {
			...
			expect(await tree.prev(13)).to.be.equal(12);
		...
		it('should give correct tress state after random insert() and root removal', async () => {
